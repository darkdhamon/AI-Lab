...
using System.IO;
using SkiaSharp;
using QRCoder; // Open source library to handle QR Code generation
...
namespace QRCodeGeneratorLib
{
    ...
    public class QRCodeGeneratorTool
    {
        ...
        {
            string userPicturesFolder = Environment.GetFolderPath(Environment.SpecialFolder.MyPictures);
        ...
        /// <param name="outputFilePath">The file path where the QR code image will be saved. If null or empty, it will use the default path.</param>
        ...
        public bool GenerateQRCode(string text, string? outputFilePath = null, int pixelsPerModule = 20, SKColor? foregroundColor = null, SKColor? backgroundColor = null, string? logoFilePath = null)
        {
            try
            {
                ...
                SKColor fgColor = foregroundColor ?? SKColors.Black;
                SKColor bgColor = backgroundColor ?? SKColors.Transparent;

                using (QRCodeGenerator qrGenerator = new QRCodeGenerator())
                {
                    QRCodeData qrCodeData = qrGenerator.CreateQrCode(text, QRCodeGenerator.ECCLevel.Q);

                    using (var surface = SKSurface.Create(new SKImageInfo(pixelsPerModule * qrCodeData.ModuleMatrix.Count, pixelsPerModule * qrCodeData.ModuleMatrix.Count)))
                    {
                        ...
                        for (int x = 0; x < qrCodeData.ModuleMatrix.Count; x++)
                        {
                            for (int y = 0; y < qrCodeData.ModuleMatrix[x].Count; y++)
                            {
                                ...
                                {
                                    canvas.DrawRect(new SKRect(x * pixelsPerModule, y * pixelsPerModule, (x + 1) * pixelsPerModule, (y + 1) * pixelsPerModule), new SKPaint { Color = fgColor });
                                }
                            }
                        }
                        ...
                        if (!string.IsNullOrWhiteSpace(logoFilePath) && File.Exists(logoFilePath))
                        {
                            ...
                            {
                                int logoSize = pixelsPerModule * qrCodeData.ModuleMatrix.Count / 5; // Logo size is 1/5th of QR code
                                int logoX = (pixelsPerModule * qrCodeData.ModuleMatrix.Count - logoSize) / 2;
                                int logoY = (pixelsPerModule * qrCodeData.ModuleMatrix.Count - logoSize) / 2;
        ...
        /// <param name="logoFilePath">Optional file path to a logo image to embed in the center of the QR code.</param>
        /// <returns>Returns true if the QR code was successfully generated and saved, otherwise false.</returns>
        public bool GenerateHyperlinkQRCode(string url, string? outputFilePath = null, int pixelsPerModule = 20, SKColor? foregroundColor = null, SKColor? backgroundColor = null, string? logoFilePath = null)
        {
        ...
        /// <param name="logoFilePath">Optional file path to a logo image to embed in the center of the QR code.</param>
        /// <returns>Returns true if the QR code was successfully generated and saved, otherwise false.</returns>
        public bool GenerateContactCardQRCode(string firstName, string lastName, string organization, string phone, string email, string? outputFilePath = null, int pixelsPerModule = 20, SKColor? foregroundColor = null, SKColor? backgroundColor = null, string? logoFilePath = null)
        {
        ...
        /// <param name="logoFilePath">Optional file path to a logo image to embed in the center of the QR code.</param>
        /// <returns>Returns true if the QR code was successfully generated and saved, otherwise false.</returns>
        public bool GenerateEventQRCode(string eventName, DateTime startTime, DateTime endTime, string location, string description, string? outputFilePath = null, int pixelsPerModule = 20, SKColor? foregroundColor = null, SKColor? backgroundColor = null, string? logoFilePath = null)
        {
            string eventDetails = $"BEGIN:VEVENT\n" +
                                  $"SUMMARY:{eventName}\n" +
            ...
            return GenerateQRCode(eventDetails, outputFilePath, pixelsPerModule, foregroundColor, backgroundColor, logoFilePath);
        }
        ...
        /// <param name="logoFilePath">Optional file path to a logo image to embed in the center of the QR code.</param>
        /// <returns>Returns true if the QR code was successfully generated and saved, otherwise false.</returns>
        public bool GenerateSocialMediaQRCode(SocialMediaPlatform platform, string usernameOrURL, string? outputFilePath = null, int pixelsPerModule = 20, SKColor? foregroundColor = null, SKColor? backgroundColor = null, string? logoFilePath = null)
        {
            ...
            return GenerateQRCode(socialMediaURL, outputFilePath, pixelsPerModule, foregroundColor, backgroundColor, logoFilePath);
        }
